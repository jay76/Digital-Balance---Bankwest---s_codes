...
function s_doPlugins(s) {
    ...
    if (s._pageNameFlag) {
        ...
        else if (s.pageName == "personal:landing page" || (s.pageName == "business:landing page" || s.pageName == "sb:landing page")) {
            s_pageHolder = s.getPageName();
            s.pageName += ":" + s_pageHolder;
        }
        if (!s.channel) {
            s.channel = "bw";
            s.prop46 = window.location.hash;
            s.prop47 = 'D=g+"' + window.location.hash + '"';
        }
        if (s.pageName == "?") {s.pageName = "";}
        if (s.pageName.length > 0) {s.pageName = s.channel + ":" + s.pageName;}
        else {
        	s.pageName = s.channel;
        }
        s.pageName = s.pageName.toLowerCase();
        s.pageName = s.pageName.replace("?", "");
        var pgNmSp = s.pageName.split(":");
        var pgNmHold = pgNmSp.slice(0, 3).join(":");
        var pgNmShrtn = pgNmSp.slice(3).join(":");
        if (s.pageName.match(/^bw:business:/gi)) {
            if (pgNmHold == "bw:business:business-deposits") {pgNmShrtn = pgNmShrtn.replace("business-deposit-", "");}
            if (pgNmHold == "bw:business:international-trade") {pgNmShrtn = pgNmShrtn.replace("international-trade-", "");}
            pgNmShrtn = pgNmShrtn.replace("business-", "");
        }
        if (s.pageName.match(/^bw:sb:small-business:/gi)) {
            if (pgNmHold ==
                "bw:sb:small-business:business-deposits") {pgNmShrtn = pgNmShrtn.replace("business-deposit-", "");}
            if (pgNmHold == "bw:sb:small-business:international-trade") {pgNmShrtn = pgNmShrtn.replace("international-trade-", "");}
            pgNmShrtn = pgNmShrtn.replace("business-", "");
        }
        if (pgNmHold == "bw:sb:small-business") {pgNmHold = pgNmHold.replace(":small-business", ":sb");}
        if (s.pageName.match(/^bw:personal:/gi)) {
            if (pgNmHold == "bw:personal:investments-advice") {
                pgNmShrtn = pgNmShrtn.replace("personal-", "");
                pgNmShrtn = pgNmShrtn.replace("-advice","");
                pgNmShrtn = pgNmShrtn.replace("help-with-financial-strategies", "help-with-strategies");
            }
            if (pgNmHold == "bw:personal:credit-cards") {pgNmShrtn = pgNmShrtn.replace("credit-card-", "");}
            if (pgNmHold == "bw:personal:everyday-accounts") {
                pgNmShrtn = pgNmShrtn.replace("everyday-", "");
                pgNmShrtn = pgNmShrtn.replace("-accounts", "");
                pgNmShrtn = pgNmShrtn.replace("-account", "");
            }
            if (pgNmHold == "bw:personal:home-loans") {
                pgNmShrtn = pgNmShrtn.replace("-home-loan", "");
                pgNmShrtn = pgNmShrtn.replace("home-loan-", "");
            }
            if (pgNmHold == "bw:personal:our-services") {pgNmShrtn = pgNmShrtn.replace("information-for-existing-customers", "existing-customers");}
            if (pgNmHold == "bw:personal:personal-loans") {
                pgNmShrtn = pgNmShrtn.replace("-personal-", "");
                pgNmShrtn = pgNmShrtn.replace("personal-loan-", "");
                pgNmShrtn = pgNmShrtn.replace("personal-loans-", "");
            }
            if (pgNmHold == "bw:personal:savings-term-deposits") {
                pgNmShrtn = pgNmShrtn.replace("savings-accounts", "savings");
                pgNmShrtn = pgNmShrtn.replace("-savings-term", "");
                pgNmShrtn = pgNmShrtn.replace("term-deposits", "td");
                pgNmShrtn = pgNmShrtn.replace("term-deposit","td");
                pgNmShrtn = pgNmShrtn.replace("savings-and-tds-", "");
            }
        }
        if (pgNmShrtn && pgNmShrtn.length > 0) {pgNmShrtn = ":" + pgNmShrtn;}
        s.pageName = pgNmHold + pgNmShrtn;
        s.pageName = s.pageName.replace("--", "-");
        s.pageName = s.pageName.replace("::", ":");
        s.pageName = s.pageName.replace("::", ":");
        ...
        if (valueArray[1] == valueArray[2]) {
            valueArray[1] = "";
            s.pageName = valueArray.join(":");
            s.pageName = s.pageName.replace("::", ":");
            valueArray = s.pageName.split(":");
        }
        s.prop1 = valueArray.slice(0, 3).join(":");
        s.prop2 = valueArray.slice(0, 4).join(":");
        s.prop3 = valueArray.slice(0, 5).join(":");
        s.hier1 = s.pageName + ":index";
        s.prop8 = s.eVar8 = "D=pageName";
        s._pageNameFlag = false;
    }
    if (typeof s.linkType == "undefined") {s.events = s.apl(s.events, "event13", ",", 2);}
    ...
    s.eVar10 = s.c_r("BWAB");
    s.prop5 = "";
    if (s.eVar9.length > 0) {s.prop5 = "Retail";}
    if (s.eVar10.length > 0) {s.prop5 += "Business";}
    if (s.prop5 == "RetailBusiness") {s.prop5 = "Retail And Business";}
    if (s.prop5 == "") {s.prop5 = "Unidentified";}
    if (s.getVisitStart("s_gVs_p25")) {s.prop31 = s.pageName;}
    if (typeof s.linkType == "undefined" && s.pageName == "bw:business") {s.eVar31 = "Business Homepage|Other Links";}
    ...
    s.eVar75 = sVerNum;
    s.getGoogleRank("event35", "event36", "", "eVar36");
    s.eVar13 = s.getQueryParam("applynow");
	...
	if (s_site) {s.prop42=s.eVar42=s_site;}
...
// START LAST PRODUCT TRACKING
/**
 * Last product tracking.
 * Tracks the last product viewed when a user visits one of the URLs stored in formURLs.
 */
...
/**
 * @function rememberLastProduct()
 * Whenever a page with a product string is visited, this method will drop a cookie to remember it (called "lastProduct").
 * On any of the pages listed in var formURLs, the function will set s.products to the value stored in this cookie.
 * @augments s
 * @returns {String} The product string found inside cookie:lastProduct (if any). Use not mandatory in most cases, as s.products is automatically assigned this value.
 * @version 1.0 
 */
s.rememberLastProduct = function() {

	var lastProduct;

	// list of all enquiry form pages that should run the last product tracking.
	var formURLs = [
		"https://secureforms.bankwest.com.au/retail-forms/home-loan-enquiry",
		"https://secureforms.bankwest.com.au/retail-forms/home-loan-top-up-enquiry",
		"https://secureforms.bankwest.com.au/retail-forms/home-loan-enquiry3",
		"http://www.bankwest.com.au/retail-forms/home-loan-specialists",
		"https://secureforms.bankwest.com.au/retail-forms/employee-benefits",
		"https://secureforms.bankwest.com.au/find-my-rm-forms/bcrc-form-lrl",
		"https://secureforms.bankwest.com.au/business-forms/business-banking-enquiry",
		"https://secureforms.bankwest.com.au/find-my-rm-forms/bcrc-form",
		"https://secureforms.bankwest.com.au/find-my-rm-forms/bcrc-form-lrl",
		"https://secureforms.bankwest.com.au/cs/Satellite",
		"http://tt.digitalbalance.com.au/bankwest/lastProductCookie/enquiry.php"
	];
	
	// if s.products is defined, store it a cookie.
	if(typeof s.products !== "undefined" && s.products != "") {
		docCookies.setItem("lastProduct",s.products,Infinity,"/","bankwest.com.au");
		// write cookie with s.products
	}
	
	// if we are currently on one of the defined form pages, and there is no product string, set s.products to the cookie value.
	if((formURLs.indexOf(document.location.href) != -1) && (typeof s.products === "undefined" || s.products == ""))
	{
		lastProduct = docCookies.getItem("lastProduct");
		s.products = lastProduct;
		return lastProduct;
	}
}
// END rememberLastProduct().
...
s.searchReferrer = function(currentData) {
	...
	for(var y = 0; y < s.channelData[currentData][2].length; y++){
		...
		if((refr != null) && (refr.host.replace("www.","").indexOf(s.channelData[currentData][2][y]) == 0)) {
...
s.getCodeObject = function(){
	...
	//console.log("Ex:" + existingSession);
